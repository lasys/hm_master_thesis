...
def execute_with_cdr(
    ...
    **kwargs: Any,
    ...
    # Batching! -> executor can handle multiple circuits 
    # ziemlich Array verliebt der Mitiq-Coder... schlimm..
    # Batching ist nun eingebaut. executor kann mit einer Liste von circuits umgehen. 
    noisy_counts = np.array(
        [[counts] for counts in executor([c[0] for c in all_circuits])]
    )
