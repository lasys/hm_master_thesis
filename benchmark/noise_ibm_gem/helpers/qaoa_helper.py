...
#from shared.no_opt_qaoa import No_Opt_QAOA
...
def _run_qaoa_evaluation(max_cut, qaoa, gem):
    algorithm = GEMMinimumEigenOptimizer(qaoa, gem)
    result = algorithm.solve(max_cut.to_qubo())
    mean, r, ar = max_cut.analyse_evaluation(result, print_output=False)
    return mean, r, ar
    
def start_qaoa_evaluation(max_cut, eval_num, reps, gem, maxiter=50, init_points=None):
    means = []
    ratios = []
    approx_ratios = []
    print(f"p={reps}: ",end='')
    for i in range(0, eval_num):
        ...
        try:
            mean,r,ar = _run_qaoa_evaluation(max_cut, qaoa, gem)
            means.append(mean)
            ratios.append(r)
