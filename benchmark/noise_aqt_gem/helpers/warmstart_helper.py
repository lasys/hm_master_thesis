...
def run_ws_qaoa(max_cut, qaoa, gem, epsilon=0.25,print_output=True):
    ...
    if print_output:
        ...
        print(f"Run GEMWarmStartQAOAOptimizer with epsilon: {epsilon}")
        max_cut.plot_histogram(distribution, mean)
        
    return result, mean, optimal_parameters


def run_evaluation_ws_qaoa(max_cut, qaoa, gem, epsilon=0.25, print_output=False):
    result, mean, optimal_parameters = run_ws_qaoa(max_cut, qaoa, gem, epsilon, print_output=print_output)
    mean, r, ar = max_cut.analyse_evaluation(result, print_output=False)
    return mean, r, ar


def start_ws_qaoa_evaluation(max_cut, eval_num, reps, gem, epsilon=0.25, maxiter=50):
    ...
    for i in range(0, eval_num):
        ...
        try:
            mean,r,ar = run_evaluation_ws_qaoa(max_cut, qaoa, gem, epsilon=epsilon)
            means.append(mean)
            ratios.append(r)
