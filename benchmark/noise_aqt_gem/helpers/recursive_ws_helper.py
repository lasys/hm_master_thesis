...
def _run_recursive_ws_qaoa(max_cut, qaoa, gem, epsilon=0.25):
    ws_qaoa = GEMWarmStartQAOAOptimizer(pre_solver=GoemansWilliamsonOptimizer(5),
                                     num_initial_solutions=5, gem=gem, warm_start_factory=MyWarmStartQAOAFactory(epsilon),
...
def run_recursive_ws_qaoa(max_cut, qaoa, gem, epsilon=0.25, print_output=False):
    result, optimal_parameters = _run_recursive_ws_qaoa(max_cut, qaoa, gem, epsilon)
...
def _run_evaluation_recursive_ws_qaoa(max_cut, qaoa, gem, epsilon=0.25, print_output=False):
    result, optimal_parameters = _run_recursive_ws_qaoa(max_cut, qaoa, gem, epsilon)
    mean, r, ar = max_cut.analyse_evaluation(result, print_output=print_output)
    return mean, r, ar


def start_recursive_ws_qaoa_evaluation(max_cut, eval_num, reps, epsilon, gem, maxiter=50):
    ...
    for i in range(0, eval_num):
        ...
        try:
            mean,r,ar = _run_evaluation_recursive_ws_qaoa(max_cut, qaoa=qaoa, gem=gem, epsilon=epsilon)
            means.append(mean)
            ratios.append(r)
